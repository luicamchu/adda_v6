...
public class Datos1 {
	...
	public static Set<Integer> getCualidadesInt(Integer i) {
		return IntStream.range(0, getNumCualidades())
			.filter(j-> getTieneCualidad(i, j) > 0).boxed().collect(Collectors.toSet());
	}
	
	// Cambia Boolean por Integer
	...
	// Cambia Boolean por Integer
	public static Integer getSonIncompatibles(Integer i, Integer j) {
	    return candidatos.get(i).incompatibilidades().contains(j) ? 1 : 0;
	}
